...
class KMeans:
    ...
    def reassignClusters(self, dataSet, c, clusters):
        ...
        for point in dataSet:
            ...
            for cluster in clusters:  #remove the point from it's current cluster even if it should be in it
                if point in cluster:
                    cluster.remove(point)
            clusters[minIndex].add(point) #add the current point to the cluster closest to it determined by Euclidean distance above
    # -------------------------------------------------------------------
    ...
    def getCentroid(self, cluster):
        ...
        size = len(cluster)
        for point in cluster: #add up all the x and y values of each point in the cluster
            cx += point.x
            cy += point.y
        #divide the total x and total y sum by the number of points in the cluster i.e find the average
        cx /= size
        cy /= size
