...
class DataPoints:
    ...
    def getMean(clusters, mean):
        ...
        for i in range(len(clusters)):
            ...
            mean[i][0] = x_total / len(clusters[i])
            mean[i][1] = y_total / len(clusters[i])
            x_total = 0.0
            y_total = 0.0
    # -------------------------------------------------------------------
    @staticmethod
    def getStdDeviation(clusters, mean, stddev):
        ...
        for i in range(len(clusters)):
            for point in clusters[i]:
                x_total += pow((point.x - mean[i][0]), 2)
                y_total += pow((point.y - mean[i][1]), 2)
            
            stddev[i][0] = math.sqrt(x_total / (len(clusters[i]) - 1))
            stddev[i][1] = math.sqrt(y_total / (len(clusters[i]) - 1))
            x_total = 0.0
            y_total = 0.0
    # -------------------------------------------------------------------
    @staticmethod
    def getCovariance(clusters, mean, stddev, cov):
        ...
        for i in range(len(clusters)):
            for point in clusters[i]:
                total += (point.x - mean[i][0]) * (point.y - mean[i][1])
            
            cov[i][0][1] = total / (len(clusters[i]) - 1)
            cov[i][1][0] = total / (len(clusters[i]) - 1)
